...
impl std::fmt::Display for Data {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.do_this {
            f.write_fmt(format_args!("{0}{1}{2}", "This then ", self.action, " :D"))?;
        } else if self.do_that {
            f.write_fmt(format_args!("{0}{1}{2}", "That then ", self.action, " :D"))?;
        } else {
            f.write_fmt(format_args!("{0}{1}{2}", "Can\'t ", self.action, " :("))?;
        }
        Ok(())
    }
}
...
pub const test_if: test::TestDescAndFn = test::TestDescAndFn {
    desc: test::TestDesc {
        ...
        source_file: "tests\\if-elseif-else.rs",
        start_line: 19usize,
        start_col: 4usize,
        end_line: 19usize,
        end_col: 11usize,
